...
function saveCalculation() {
    if (!window.currentCalc) {
        notify.error('Error', 'Por favor, realiza un cálculo primero');
        return;
    }
    ...
    notify.success('¡Éxito!', 'Cálculo guardado correctamente');
}

async function showLoginWarning() {
    const confirmed = await ConfirmationDialog.show({
        title: 'Inicia Sesión',
        message: 'Para guardar tus cálculos debes iniciar sesión. ¿Deseas ir al login ahora?',
        icon: '🔐',
...
async function showGuestWarning() {
    const confirmed = await ConfirmationDialog.show({
        ...
        message: 'Los invitados no pueden guardar cálculos. Crea una cuenta para acceso completo. ¿Deseas crear una cuenta ahora?',
        icon: '📝',
...
function exportCalculation() {
    ...
    notify.success('¡Descarga Completada!', `${data.projectName}.csv descargado correctamente`);
}
